...
namespace std {
    template <>
    struct hash<misc::GameModel::Vertex> {
        size_t operator()(misc::GameModel::Vertex const& vertex) const {
            size_t seed = 0;
            misc::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
            return seed;
        }
    };
}  // namespace std
...
namespace misc {
    ...
    GameModel::GameModel(GameDevice& device, const GameModel::Builder& builder) : myDevice{ device } {
    ...
    std::unique_ptr<GameModel> GameModel::createModelFromFile(
    ...
    void GameModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
        ...
        vertexBuffer = std::make_unique<GameBuffer>(
            myDevice,
            vertexSize,
            vertexCount,
    ...
    void GameModel::createIndexBuffers(const std::vector<uint32_t>& indices) {
        ...
        indexBuffer = std::make_unique<GameBuffer>(
            myDevice,
            indexSize,
            indexCount,
